...
def writeSbatchHeader(projectID, cores, time,
                      sampleName, sbatchName):
    '''
...
def writeSbatchScript(outputFile, jobID, projectID,
                      cores, time, jobName,
                      scriptName, dependency="none"):
    '''
    Writes a script line to submit sbatch job and takes into account
...
def writeMapBWAJob(fileNameR1list, filePathR1list, outputFile,
                   reference, sample, laneID,
                   cores, ram):
    '''
    ...
    outputFile.write(
        "\nbwa mem -t %s -M "
        "-R '@RG\\tID:%s_%s\\tPL:illumina\\tLB:%s\\tSM:%s' "
        "%s %s %s "
        "| samtools sort -O bam -@ %s -m %sG > "
        "%s_%s.bam\n" %
        (cores,
         sample, laneID, sample, sample,
         reference, filePathR1, filePathR2,
         cores, ram,
         sample, laneID))


def writeMapStampyJob(fileNameR1list, filePathR1list, outputFile,
                      divergence, reference, sample,
                      laneID, cores, ram,
                      tmp):
    '''
    ...
    outputFile.write(
        "\nstampy.py -g %s -h %s "
        "--substitutionrate=%s -t %s "
        "-o %s/%s_%s_stampy.sam -M %s %s\n"
        "samtools sort -O bam -@ %s -m %sG %s/%s_%s_stampy.sam > "
        "%s/%s_%s.bam\n"
        ...
        "RGPU=%s_%s \\\n"
        "RGSM=%s\n" %
        (referenceStampy, referenceStampy,
         divergence, cores,
         tmp, sample, laneID, filePathR1, filePathR2,
         cores, ram, tmp, sample, laneID,
         tmp, sample, laneID,
         ram,
         tmp, sample, laneID,
         sample, laneID,
...
def writeMarkDuplJob(outputFile, sample, ram,
                     tmp):
    '''
